...
const removeMember = TryCatch(
    async (req, res, next) => {
        ...
        if (chat.members.length <= 3) return next(new ErrorHandler("Group Members count should be greater than 2", 403));
...
const leaveGroup = TryCatch(
    async (req, res, next) => {
        ...
        if (remainingMembers.length < 3) return next(new ErrorHandler("You are not allowed to leave the group", 403));
...
const sendAttachments = TryCatch(
    async (req, res, next) => {
        ...
        if (files.length > 5) return next(new ErrorHandler("Attachments can't be more than 5", 400));
...
const getMessages = TryCatch(
    async (req, res, next) => {
        ...
        // Correcting Promise.all structure
        const [messages, totalMessagesCount] = await Promise.all([
            Message.find({ chat: chatId })
                ...
                .lean(),
            Message.countDocuments({ chat: chatId })  // Ensure this is inside the array
        ]);

        const totalPages = Math.ceil(totalMessagesCount / resultPerPage);
        // console.log(totalPages);  // totalPages will no longer be NaN
